...
class DownloadController extends Controller
{
    public function createCsv ($status, $prjid) {
        ...
        $csv->insertOne(['Project ID', 'Respondent ID', 'Vendor', 'Status', 'Country', 'City', 'IP', 'Start Time', 'End Time', 'LOI (Seconds)']);
        foreach ($results as $result) {
            ...
            $csv->insertOne([$result->projectid, $result->respid, $result->vendor, $result->status, $result->Languageid, $result->city, $result->IP, $starttime, $endtime, $loi]);
        }
